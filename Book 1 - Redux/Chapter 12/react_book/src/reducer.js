...
function cartReducer(state, action) {
  ...
  switch (action.type) { //The only time we use this is if we have a lot of things to check. the switch chase one value (case) at a time. We are using it here because we have 2 things that need to be done, so if we call either of the "functions", only one of them will run. 
    case "addProduct":
      return {
